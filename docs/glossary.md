...
| **Kind style** | Visual tokens for a block kind (`fontSize`, `color`, `fontWeight`, `fontStyle`, optional `textAlign` / `initialCap` / `indicate` / `htmlTag`). Built-in defaults plus the open library’s `styles.json`. | CSS theme, stylesheet |
